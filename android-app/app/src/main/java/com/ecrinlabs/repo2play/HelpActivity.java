...
public class HelpActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        add(page,"REAL BRANCH EXAMPLE","During our Android Simple Calculator test, we first entered:\n\nmain\n\nThe build failed because the repository actually used:\n\nmaster\n\nAfter changing the branch to master, Repo2Play successfully built a working APK.\n\nIf a build fails very early, check the branch first.");
        ...
        add(page,"UPDATE BUILD","Choose UPDATE when building a newer version of the SAME application.\n\nExample:\n\nVersion 1 → NEW\nVersion 2 → UPDATE\nVersion 3 → UPDATE\n\nUPDATE should use the same correct signing identity.\n\nIf the original key is unavailable, the update may not be installable over the previous version or accepted as the same application.");
        add(page,"IMPORT ORIGINAL JKS","Use IMPORT ORIGINAL JKS when the application was originally signed elsewhere or when the correct original signing key is not already stored.\n\nImport ONLY the original key belonging to that application.\n\nDo not use a random JKS or a key from another app.");
